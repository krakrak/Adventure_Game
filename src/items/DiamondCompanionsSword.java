 ...
 * @author Irakiza Krak
 */
public class DiamondCompanionsSword extends AbstractWeapon {
    
    private Random rdm = new Random();
    private Double removeNbLives;
    private int nbLivesRemain;
    
    public DiamondCompanionsSword(){
        super("Diamond Companion's Sword", "Large Diamond two-handed sword for the leader of the Companion's", "Di s");
    }
    ...
    public GameState use(Game game) {
        ...
        for(Npc creature : npcs){
            if(creature instanceof Killable1 || creature instanceof Killable || creature instanceof Killable2 || creature instanceof Killable3){
        ...
        if(killablenpc != null){
            if (!History.getHistory().isLoaded()){
                ...
                if(nbLivesRemain > 0 ){
                    removeNbLives = rdm.nextDouble()*(100/nbLivesRemain) + 17;
